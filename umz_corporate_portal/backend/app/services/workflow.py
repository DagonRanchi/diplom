...
HR_STATUSES = {
    ...
    ApplicationStatus.hr_review.value,
    ApplicationStatus.archived.value,
    ApplicationStatus.rejected.value,
}
DOCUMENT_STATUSES = {
    ApplicationStatus.approved_by_hr.value,
    ApplicationStatus.document_review.value,
    ApplicationStatus.manager_review.value,
...
def query_applications_for_user(db: Session, user: User):
    ...
    if user.role == Role.hr_admin.value:
        return query.filter(Application.status.in_(HR_STATUSES | {ApplicationStatus.approved_by_hr.value}))
    if user.role == Role.document_admin.value:
        return query.filter(Application.status.in_(DOCUMENT_STATUSES))
...
def can_view_application(user: User, app: Application) -> bool:
    if user.role in {Role.tech_admin.value, Role.assistant.value}:
        return True
    if user.role == Role.hr_admin.value:
        return app.status in HR_STATUSES or app.status == ApplicationStatus.approved_by_hr.value
    if user.role == Role.document_admin.value:
        return app.status in DOCUMENT_STATUSES
    if user.role == Role.department_manager.value:
...
def reject_application(db: Session, app: Application, reason: str, rejected_by: User) -> None:
    ...
    rejected_folder = ensure_folder_path(db, ["Отклоненные документы"])
